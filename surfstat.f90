  subroutine surfstat
     ...
     real(dp) :: emin, emax, w, k(2), time_start, time_end, s0(3), s1(3), time_start_inner, time_end_inner
    ...
    !ENDDO
     ...
     do ikp= 1+cpuid, knv2, num_cpu
        ...
        time_start_inner= 0d0
        time_end_inner=0d0
        do j = 1, omeganum
           if (cpuid==0.and. mod(ikp/num_cpu, 10)==0) &
              write(stdout, *) "SurfaceSSInner, j", j, omeganum, "time left", &
              (time_end_inner- time_start_inner)*(omeganum-j), ' s'

           call now(time_start_inner)
           ...
           enddo ! i
            ...
            enddo ! i
            call now(time_end_inner)
        enddo ! j
        write(stdout, *) 'SurfaceSS iter end'
        call now(time_end)
     enddo ! ikp
